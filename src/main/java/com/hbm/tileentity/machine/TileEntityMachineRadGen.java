...
public class TileEntityMachineRadGen extends TileEntityMachineBase implements IEnergyProviderMK2, IGUIProvider, IInfoProviderEC {
	...
	static {
		...
		for(int i = 0; i < ItemWasteShort.WasteClass.values().length; i++) {
			fuels.put(	new ComparableStack(ModItems.nuclear_waste_short, 1, i),		new Triplet<Integer, Integer, ItemStack>(1500,	150 * 20,	new ItemStack(ModItems.nuclear_waste_short_depleted, 1, i)));
			fuels.put(	new ComparableStack(ModItems.nuclear_waste_short_tiny, 1, i),	new Triplet<Integer, Integer, ItemStack>(150,	15 * 20,		new ItemStack(ModItems.nuclear_waste_short_depleted_tiny, 1, i)));
		}
		for(int i = 0; i < ItemWasteLong.WasteClass.values().length; i++) {
			fuels.put(	new ComparableStack(ModItems.nuclear_waste_long, 1, i),		new Triplet<Integer, Integer, ItemStack>(500,	600 * 20,	new ItemStack(ModItems.nuclear_waste_long_depleted, 1, i)));
			fuels.put(	new ComparableStack(ModItems.nuclear_waste_long_tiny, 1, i),	new Triplet<Integer, Integer, ItemStack>(50,		60 * 20,		new ItemStack(ModItems.nuclear_waste_long_depleted_tiny, 1, i)));
		}
